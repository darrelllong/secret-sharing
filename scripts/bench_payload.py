...
def main():
    ...
    ap.add_argument(
        "--pilot", type=Path, default=Path.home() / "pilot-bench/build/cli/bench"
    )
    ...
    metadata = {
        ...
        "preset": args.preset,
        "target_sample_ms": 20,
        "pilot_commit": command(
            ["git", "-C", str(args.pilot.resolve().parents[2]), "rev-parse", "HEAD"]
        ),
        ...
        for size in [64, 1024]:
            for phase in ["split", "reconstruct"]:
                ...
                for implementation in implementations:
                    ...
                    binary = (
                        ROOT / "target/release/pilot_payload"
                        if implementation == "rust"
                        else args.cpp.resolve()
                    )
                    cmd = [
                        "taskset",
                        "-c",
                    ...
                    try:
                        p = subprocess.run(
                            ...
                            text=True,
                            env=env,
                            timeout=args.limit + 120,
                        )
                        log = p.stdout + "\n" + p.stderr
                        (output / (label + ".log")).write_text(log)
                        exported = output / label / "pi_results.csv"
                        mean = ci = rounds = None
                        if exported.exists():
                            with exported.open() as source:
                                result = next(csv.DictReader(source))
                            if (
                                result["readings_mean"]
                                and result["readings_subsession_ci"]
                            ):
                                mean = float(result["readings_mean"])
                                ci = float(result["readings_subsession_ci"])
                                rounds = int(result["readings_num"])
                        row.update(
                            status=(
                                "ok"
                                if p.returncode == 0
